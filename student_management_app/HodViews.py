...
def add_staff_save(request):
    if request.method != "POST":
        ...
        if 'teacher_license' in request.FILES:
            ...
            upload_result = cloudinary.uploader.upload(teacher_license, folder="media")
            teacher_license_url = upload_result.get('url')

        if 'signature' in request.FILES:
            signature = request.FILES['signature']
            upload_result = cloudinary.uploader.upload(signature, folder="media")
        ...
        try:
            ...
            # Only assign URLs if files were uploaded
            if profile_pic_url:
                user.staffs.profile_pic = profile_pic_url
            if teacher_license_url:
...
def edit_staff(request, staff_id):
    ...
    if staff.address:
        ...
        if len(address_parts) == 4:
            street, barangay, city, region = address_parts
    ...
    context = {
        ...
        "registration_date": staff.registration_date
    }
